...
// Return histogram of grayscale image
    ...
    public static void rotateleft_023(Picture p) {
        Picture target = new Picture(p.getHeight(), p.getWidth());
        Pixel sourcePixel = null;
        Pixel targetPixel = null;
        for (int sourceX = 0; sourceX < p.getWidth(); sourceX++) {
            for (int sourceY = 0; sourceY < p.getHeight(); sourceY++) {
                ...
                targetPixel = target.getPixel(sourceY, p.getWidth() - 1 - sourceX);
                targetPixel.setColor(sourcePixel.getColor());
            }
    ...
    public static void rotateRight_023(Picture p) {
        Picture target = new Picture(p.getHeight(), p.getWidth());
        Pixel sourcePixel = null;
        Pixel targetPixel = null;
        ...
        for (int x = 0; x < p.getWidth(); x++) {
            ...
            for (int y = 0; y < p.getHeight(); y++) {
                // set the target pixel color to the source pixel color
                sourcePixel = p.getPixel(x, y);
                targetPixel = target.getPixel(p.getHeight() - 1 - y, x);
                targetPixel.setColor(sourcePixel.getColor());
            }
        }
        target.show();
    ...
    public static void reflection_023(Picture p) {
        ...
        for (int x = 0; x < p.getWidth(); x++) {
            for (int y = 0; y < mirrorPoint; y++) {
                ...
                bottomPixel = p.getPixel(x, height - 1 - y);
                topPixel.setColor(bottomPixel.getColor());
            }
        }
        p.show();
    ...
    public static void blendPictures_023(Picture picture1) {
        String fileName2 = FileChooser.pickAFile();
        Picture picture2 = new Picture(fileName2);
        //to fix the index out bounds error we create picture with sum if height from the two picture and the with also but after 
        //the bend extar white pixel well be thare so we will fix it in the last after ////// line
        Picture TargetPicture = new Picture(picture1.getWidth() + picture2.getWidth(), picture1.getHeight() + picture2.getHeight());
        Pixel pic1;
        Pixel pic2;
        Pixel targetPixel;
        int sourceX = 0;
        ...
        for (; sourceX < 150; sourceX++, targetX++) {
            for (int sourceY = 0, targetY = 0; sourceY < picture1.getHeight(); sourceY++, targetY++) {
                pic1 = picture1.getPixel(sourceX, sourceY);
                targetPixel = TargetPicture.getPixel(targetX, targetY);
        ...
        for (; sourceX < picture1.getWidth(); sourceX++, targetX++) {
            for (int sourceY = 0, targetY = 0; sourceY < picture1.getHeight(); sourceY++, targetY++) {
                ...
                targetPixel = TargetPicture.getPixel(targetX, targetY);
                Color b = new Color((int) (pic1.getRed() * 0.5 + pic2.getRed() * 0.5), (int) (pic1.getGreen() * 0.5 + pic2.getGreen() * 0.5),
                        (int) (pic1.getBlue() * 0.5 + pic2.getBlue() * 0.5));
        ...
        ////////////
        //we get the height and Width from nested lopp above and assgin it in tmpx and tmpy
        //after we get the actule photo after the bend i make new target pixter with the tmpx and tmpy size
        //i capy pixel from old targetpicture to the new targetpicture with out the extar white pixel
        Pixel tmpc = null;
    ...
    private static void Flip_Hor_276(Picture pic1) {
        Picture sourcePicture = new Picture(pic1);
        ...
        /* for (int sourceX = 0; sourceX < sourcePicture.getWidth(); sourceX++) {
            for (int sourceY = 0; sourceY < sourcePicture.getHeight() / 2; sourceY++) {
        ...
        for (int sourceX = 0; sourceX < TargetPicture.getWidth(); sourceX++) {
            for (int sourceY = TargetPicture.getHeight() / 2; sourceY < TargetPicture.getHeight(); sourceY++) {
                sourcePixel = sourcePicture.getPixel(sourceX, sourceY);
                targetX = sourcePicture.getWidth() - 1 - sourceX;
    ...
    public static void negate_023(Picture Picture) {

        Pixel[] pixelArray = Picture.getPixels();

        Pixel pixel;

        for (Pixel pixelArrayAfter : pixelArray) {

            pixel = pixelArrayAfter;
    ...
    public void changeColorsLab5_023(double red, double green, double blue) {
        Pixel[] pixelArray = this.getPixels();
        double value = 0;
        for (Pixel pixelObj : pixelArray) {
            if (red != -1.0) {
